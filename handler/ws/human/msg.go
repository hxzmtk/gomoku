...
func (msg *Msg) Receive() {
	...
	case roomMsg:
		...
		switch m.Action {
		case RoomCreate:
			if roomNumber, err := hub.Hub.CreateRoom(c); err == nil {
			...
			c.Send <- msg
		case RoomJoin:
			if err := hub.Hub.JoinRoom(c, m.RoomNumber); err != nil {
				msg.Msg = err.Error()
				msg.Status = false
		...
		case RoomWatch:
			roomNumber := m.RoomNumber
			if room := hub.Hub.GetRoomByID(uint(roomNumber)); room != nil {
				...
				xy := room.GetChessBoardState().NoNilHand()
				mm := ResRoomMsg{
		...
		case RoomRegretAgree:
			...
			if c.Room.NextWho == enemy && enemy != nil {
				if err := c.Room.Regret(); err != nil {
					...
					msg.Content = ResRoomMsgRegret{Action: RoomRegretAgree, XY: c.Room.GetWalks()[:2]}
					msg.Status = true
					newMsg := *msg
					newMsg.Msg = "对方同意了您的悔棋"
					enemy.(*Client).Send <- &newMsg
	...
	case chessWalk:
		...
		if m.RoomNumber <= 0 || (c.Room != nil && c.Room.ID != uint(m.RoomNumber)) {
			msg.Msg = "无效的房间号"
			msg.Content = m
			c.Send <- msg
			return
		}
		if err := c.Room.GoSet(c, m.X, m.Y); err == nil {
			m.IsBlack = c.isBlack()
			c.Room.RecordWalk(chessboard.XY{
				X:    m.X,
				Y:    m.Y,
